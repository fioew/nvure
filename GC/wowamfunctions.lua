...
local type = type;
local GetSpellBookItemInfo = C_SpellBook.GetSpellBookItemInfoo;
...
function amPlayerSpellId(name)
	...
	for i1 = 1, C_SpellBook.GetNumSpellBookSkillLines() do
		local skillLineInfo = C_SpellBook.GetSpellBookSkillLineInfo(i1)
		...
		for j = offset+1, offset+numSlots do
			...
			if spellName == name then
				skillType, spellId = spellType, id
			end
		end
	end
	...
	if spellId then
		...
		local spellName, spellSubName = spell_info.name, "";
...
function amPlayerItemId(name)
	...
	for i = 1, 23 do
		...
		if ItemId then
			spell = C_Item.GetItemInfo(ItemId)
			if spell == name then
				return ItemId;
			end
		end
	...
	for i = 0, 10 do
		bagName = C_Container.GetBagName(i);
		if bagName then
			local n = C_Container.GetContainerNumSlots(i)
			for k = 1, n do	
				ItemId = C_Container.GetContainerItemID(i, k);
				if ItemId then
					spell = C_Item.GetItemInfo(ItemId);
...
function amisspell(name,tunit,gcd,Special,isname,NOCD,typenumber,SpellLevel,temp_UnitGUID,unitguid,EnergyDemand,NoAc)
	...
	local castTime = wowam.spell.Property[name]["castTime"] or 0;
	if castTime <= 0 then
...
function amr(Unit,p,q) --目標的法力、怒氣、能量 值或百分比等。
	...
	a = UnitPower(Unit, Enum.PowerType.Mana)
	b = UnitPowerMax(Unit, Enum.PowerType.Mana)
	c= b-a;
...
function amisarena()
	--return IsActiveBattlefieldArena() or false
	return false
end
...
function amSpellCooldown(spell)
	...
	local a,b,c = spellCoolDown.startTime, spellCoolDown.duration, spellCoolDown.isEnabled

	-- local a,b,c = C_Spell.GetSpellCooldown(spell)
...
function amGetSpellName(spellId)
	...
	local spellName, spellRank = spellInfo.name, nil
		...
		if ammouseLastExecuted~= nil then
			if GetTime()-ammouseLastExecuted>0.75 then
				if not IsMouseButtonDown(1) then
					C_Macro.RunMacroText("/click MouseButtonDown\n/click MouseButtonUp")
				end
...
local function am_CastSpellFrame_OnEvent(self, event, ...)
	...
	elseif UnitIsUnit("player",arg1) and ((event=="UNIT_SPELLCAST_STOP") or (event=="UNIT_SPELLCAST_SUCCEEDED") or (event=="UNIT_SPELLCAST_FAILED") or (event=="UNIT_SPELLCAST_INTERRUPTED")) then
		...
		local arg2_result = {}
		for part in string.gmatch(arg2, "[^-]+") do
			table.insert(arg2_result, part)
		end
		
		local DelayTbl = wowam.DelayTbl;
		-- local Spell = amGetSpellName(arg4);
		local Spell = amGetSpellName(arg2_result[6]);
		local castunit = wowam["CastSpellInf"]["spell"];
...
function amAffDotsVer120(target,spell)
	...
	if not AffDots.amtrack then
		print("|cffff0000请安装晋升堡垒专用的AffDots插件！");
		return -1;
	end
            ...
            local thisUnit = GetObjectWithIndex(i)
			      ...
			      if amSpellCooldown(spellId) == 0 and Totemname ==  ObjectName(thisUnit) and UnitHealth(thisUnit) > 0 and FHINSight(thisUnit) then   
				    ...
				    if checkpet == 1 and HasPetUI() then
				        ...
				        TargetUnit(thisUnit);
				        C_Macro.RunMacroText("/petattack");
...
---- FISH EX
local FishBaitList = {
	{ ID = 110274, Buff = 158031, Name = '无颚潜鱼鱼饵' },
	{ ID = 110289, Buff = 158034, Name = '塘鲈鱼饵' },
	{ ID = 110290, Buff = 158035, Name = '盲眼湖鲟鱼饵' },
	{ ID = 110291, Buff = 158036, Name = '熔火鱿鱼鱼饵' },
	{ ID = 110292, Buff = 158037, Name = '海蝎子鱼饵' },
	{ ID = 110293, Buff = 158038, Name = '深渊大嘴鳗鱼饵' },
	{ ID = 110294, Buff = 158039, Name = '黑水鞭尾鱼鱼饵' },
	{ ID = 128229, Buff = 188904, Name = '魔口狂鱼鱼饵' }
}

local FishHatList = {
	{ ID = 118380, Bonus = 100, Name = '超级捕鱼帽' },
	{ ID = 118393, Bonus = 100, Name = '带触须的帽子' },
	{ ID = 117405, Bonus = 10,  Name = '纳特的饮酒帽' },
	{ ID = 19972,  Bonus = 5,   Name = '幸运渔帽' },
	{ ID = 88710,  Bonus = 5,   Name = '纳特的帽子' },
	{ ID = 93732,  Bonus = 5,   Name = '暗月钓鱼帽' },
}

local FishHooksList = {
	{ ItemID = 118391, BuffID = 5386, Weight = 200,	ItemName = '超级鱼虫'	}, -- 10 min
	{ ItemID = 124674, BuffID = 5386, Weight = 200,	ItemName = '隔夜的油炸面圈'	}, -- 10 min
	{ ItemID = 68049,  BuffID = 4225, Weight = 150,	ItemName = '耐热疾旋型诱饵'	}, -- 15 min
	{ ItemID = 46006,  BuffID = 3868, Weight = 100,	ItemName = '发光的虫子'	}, -- 1 hour
	{ ItemID = 34861,  BuffID = 266,  Weight = 100,	ItemName = '锐利的鱼钩'	}, -- 10 min
	{ ItemID = 6533,   BuffID = 266,  Weight = 100,	ItemName = '水下诱鱼器' }, -- 10 min
	{ ItemID = 7307,   BuffID = 265,  Weight = 75,	ItemName = '食腐虫'          }, -- 10 min
	{ ItemID = 6532,   BuffID = 265,  Weight = 75,	ItemName = '明亮的小珠'	}, -- 10 min
	{ ItemID = 62673,  BuffID = 266,  Weight = 75,	ItemName = '羽状诱饵'	}, -- 10 min
	{ ItemID = 6811,   BuffID = 264,  Weight = 50,	ItemName = '水下鱼珠'	}, -- 10 min
	{ ItemID = 6530,   BuffID = 264,  Weight = 50,	ItemName = '夜色虫'	}, -- 10 min
	{ ItemID = 69907,  BuffID = 263,  Weight = 25,	ItemName = '尸虫'	}, -- 10 min
	{ ItemID = 6529,   BuffID = 263,  Weight = 25,	ItemName = '闪光的小珠'	}, -- 10 min
	{ ItemID = 67404,  BuffID = 4264, Weight = 15,	ItemName = '玻璃浮漂'	}, -- 10 min
}

local FishPoloList = {
	{ ID = 118381, Bonus = 100, Name = '日抛型鱼竿' },
	{ ID = 19970,  Bonus = 40,  Name = '奥金钓鱼竿' },
	{ ID = 84661,  Bonus = 30,  Name = '飞龙鱼竿' },
	{ ID = 116825, Bonus = 30,  Name = '狂野鱼竿' },
	{ ID = 116826, Bonus = 30,  Name = '德莱尼鱼竿' },
	{ ID = 45991,  Bonus = 30,  Name = '骨质鱼竿' },
	{ ID = 45992,  Bonus = 30,  Name = '珠宝鱼竿' },
	{ ID = 44050,  Bonus = 30,  Name = '精致的卡鲁亚克鱼竿' },
	{ ID = 45858,  Bonus = 25,  Name = '纳特的好运鱼竿' },
	{ ID = 6367,   Bonus = 20,  Name = '粗铁鱼竿' },
	{ ID = 19022,  Bonus = 20,  Name = '纳特·帕格的超级钓鱼竿FC-5000型' },
	{ ID = 25978,  Bonus = 20,  Name = '塞瑟的石墨鱼竿' },
	{ ID = 6366,   Bonus = 15,  Name = '暗木鱼竿' },
	{ ID = 84660,  Bonus = 10,  Name = '熊猫人鱼竿' },
	{ ID = 6365,   Bonus = 5,   Name = '强化钓鱼竿' },
	{ ID = 12225,  Bonus = 3,   Name = '布拉普家族鱼竿' },
	{ ID = 46337,  Bonus = 3,   Name = '斯塔斯的鱼竿' },
	{ ID = 120163, Bonus = 3,   Name = '苏克的鱼竿' },
	{ ID = 6256,   Bonus = 0,   Name = '鱼竿' },
}
---------------------------------------------------------------------------------------------------
function FHisObjectByUnit(unit,object)
	local creator = UnitCreator(object)
	return creator and creator == ObjectPointer('player')
end
---------------------------------------------------------------------------------------------------
local FishBobberCache = nil
local FishBobberList = {
	{ID = '35591'},
	{ID = '241592'},
	{ID = '241593'},
	{ID = '241594'},
...
local function FHgetBobber()
	if UnitChannelInfo("player") == nil then FishBobberCache = nil end
	if FishBobberCache and ObjectExists(FishBobberCache) then return FishBobberCache end
	for i = 1, GetObjectCount(TYPE_GAMEOBJECT), 1 do
		local Object = GetObjectWithIndex(i)
		local ObjGuid = ObjectGUID(Object) or 0
		local ObjectType, _, _, _, _, CurrentBobberID, _ = strsplit('-', ObjGuid)
		if FHisObjectByUnit("player",Object) then
			if CurrentBobberID then 
				for _,v in ipairs(FishBobberList) do
					if CurrentBobberID == v.ID then
						FishBobberCache = Object;
						return Object;
					end
				end
			end
...
local function ItemInBag(ItemID)
	...
	if ItemFound then return true, ItemCount end
	return false, 0
end
---------------------------------------------------------------------------------------------------
local function Fish_AutoBaits(wodBaits)
	if FHgetBobber() then return end
	if wodBaits == 0 then return end
	local _Bait = FishBaitList[wodBaits]
	if GetItemCount(_Bait.ID, false, false) > 0 then
		local endtime = select(7, UnitBuff('player', GetSpellInfo(_Bait.Buff)))
		if (not endtime) or endtime < GetTime() + 14 then
			UseItemByName(_Bait.ID)
		end
	end
end 
---------------------------------------------------------------------------------------------------
local function Fish_AutoHats()
	local hatsFound = {}
	for i = 1, #FishHatList do
		if GetItemCount(FishHatList[i].ID, false, false) > 0 then
			hatsFound[#hatsFound+1] = {
				ID = FishHatList[i].ID,
				Name = FishHatList[i].Name,
				Bonus = FishHatList[i].Bonus
			}
		end
	end

	if #hatsFound == 0 then return end
	...
	if #hatsFound > 0 then
		...
		if headItemID ~= bestHat.ID then
			C_Macro.RunMacroText("/equip "..bestHat.Name)
...
---------------------------------------------------------------------------------------------------
local function Fish_AutoPoles()
	local polesFound = {}
	for i = 1, #FishPoloList do
		if GetItemCount(FishPoloList[i].ID, false, false) > 0 then
			polesFound[#polesFound + 1] = {
				ID = FishPoloList[i].ID,
				Name = FishPoloList[i].Name,
				Bonus = FishPoloList[i].Bonus
			}
		end
	end
	if #polesFound == 0 then return end
	...
	if #polesFound > 0 then
		...
		if weaponItemID ~= bestPole.ID then
			C_Macro.RunMacroText("/equip "..bestPole.Name)
...
---------------------------------------------------------------------------------------------------
local HookCD = 0
local function Fish_AotuHooks()
	if FHgetBobber() then return end
	if UnitCastingInfo('player') then return true end
	if GetTime() > HookCD then
		--Fishing Poles
		if select(7, GetItemInfo(GetInventoryItemLink('player', 16))) == '鱼竿' then
			local hasEnchant, timeleft, _, enchantID = GetWeaponEnchantInfo()
			if hasEnchant and timeleft / 1000 > 15 then
				for i=1,#FishHooksList do
					if enchantID == FishHooksList[i].BuffID then
						return
					end
				end
			end
			for i=1,#FishHooksList do
				local HasItem, Count = ItemInBag(FishHooksList[i].ItemID)
				if HasItem then
					HookCD = GetTime() + 5
					UseItemByName(FishHooksList[i].ItemID)
					return true
				end
			end
		end
...
---------------------------------------------------------------------------------------------------
local BladeBoneCD = 0
local function Fish_BladeBone()
	if FHgetBobber() then return end
	if UnitCastingInfo('player') then return true end
	local expires = select(7, amaura(GetSpellInfo(182226), 'player', 2, 0))
	if expires and expires - GetTime() > 15 then return end
	local HasItem, Count = ItemInBag(122742)
	if HasItem then
		BladeBoneCD = GetTime() + 5
		UseItemByName(122742)
		return true
	end
end
---------------------------------------------------------------------------------------------------
--------------------------------fish-------------------------------
local FishLoot = false
local FishTimer = 0
function FHGoFish(...)
	if FishTimer > GetTime() then return false end
	local hat,pole,hook,bladeBone,wodBaits = ...
	if GetUnitSpeed("player") ~= 0 then return false end; 
	if UnitAffectingCombat("player") then return false end;
	if	hat ~= 0 then Fish_AutoHats() end
	if	pole ~= 0 then Fish_AutoPoles() end
	if	hook ~= 0 then Fish_AotuHooks() end
	if	bladeBone ~= 0 then Fish_BladeBone() end
	if	wodBaits ~= 0 then Fish_AutoBaits(wodBaits) end
	local BobberObject = FHgetBobber()
	if BobberObject then
		local bobbing = ObjectField(FHgetBobber(), GetOffset("CGGameObject_C__Animation"), Types.Byte)
		if bobbing == 1 then
		...
		if UnitChannelInfo("PLAYER") == nil then
			if not FishLoot then FishLoot = not FishLoot; FishTimer = 0 end
			CastSpellByName(tostring(select(1,GetSpellInfo(7620))))
			FishTimer = GetTime() + 2.4
			FishLoot = false
            return false
		end
	end
...
function FHgetSanity(unit)
	...
	if ObjectExists(unit)
		and ObjectIsUnit(unit) 
		and UnitIsVisible(unit) 
		and FHgetCreatureType(unit)
		and UnitCanAttack(unit, "player") 
		and not UnitIsDeadOrGhost(unit)
		and (UnitAffectingCombat(unit) or FHisDummy(unit))  
...
function am_RegGCD()    
    ...
    am_GCD_EVENT:SetScript("OnEvent",function(_,_,_,e,_,_,_,_,_,_,_,_,_,_)
            ...
            then  
                local _,x=C_Spell.GetSpellCooldown(61304)
                if (x>0)
                then
...
function amIskarAssistantDPS()
	...
	if UnitBuff("player",tostring(eyeanzu)) ~= nil then
	    C_Macro.RunMacroText("/click Iskar")
	    return true
	end
...
function FHIntelligentHavoc()
	local hp20 = nil
	local hp100 = nil
	local HavocCD,_=C_Spell.GetSpellCooldown(80240)
    if HavocCD == 0 then
        if #FHenemiesTable < 2 then
            return false
        end
