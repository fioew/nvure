...
function amMenu(DROPDOWNMENU_NAME_PREFIX)
	...
	local function SetGameTooltip(self,info,direction)
		...
		if info and info.Spell then
			-- local name,_,Texture = C_Spell.GetSpellInfo(info.Spell);
			local spellInfo = GetSpellInfo(info.Spell)
			local texture = spellInfo.iconID
			local spellLink,_ = C_Spell.GetSpellLink(info.Spell);
		...
		if info and info.Item then
			...
			local name, link, quality, iLevel, reqLevel, class, subclass, maxStack, equipSlot, Texture, vendorPrice = C_Item.GetItemInfo(info.Item);
	...
	local function CreateDropDownMenu(level)
		...
		local frame = CreateFrame("Frame", DROPDOWNMENU_NAME_PREFIX.."Level"..level, GetDropDownMenu(level - 1) or UIParent, BackdropTemplateMixin and "BackdropTemplate" or nil);
	...
	local function InitializeDropDownMenuSubControl(frame, subControlType)
		...
		if not editbox then
			...
			editbox = CreateFrame("EditBox", nil, frame, BackdropTemplateMixin and "BackdropTemplate" or nil)
			...
			local edge = CreateFrame("Frame", nil, editbox, BackdropTemplateMixin and "BackdropTemplate" or nil)
	...
	local editFrame = GetEditBoxMenu() or CreateFrame("Frame", DROPDOWNMENU_NAME_PREFIX.."EditBoxFrame", UIParent, BackdropTemplateMixin and "BackdropTemplate" or nil)
	...
	local sliderFrame = GetSliderMenu() or CreateFrame("Frame", DROPDOWNMENU_NAME_PREFIX.."SliderFrame", UIParent, BackdropTemplateMixin and "BackdropTemplate" or nil)
	...
	local slider = sliderFrame.childSlider or CreateFrame("Slider", nil, sliderFrame, BackdropTemplateMixin and "BackdropTemplate" or nil)
